...
const ApprovalQueue = () => {
  ...
  const handleExport = () => {
    ...
    a.setAttribute('download', 'approval-queue.csv');
      ...
      <section className="dashboard-hero" style={{ minHeight: '140px', padding: '24px 40px' }}>
        <div className="hero-content">
          <p className="hero-date" style={{ color: 'var(--text-muted)' }}>
            Dashboard &gt; <span style={{ color: 'var(--accent-color)' }}>Approval Queue</span>
          </p>
          <h1 className="hero-welcome">Approval Queue</h1>
          <p className="hero-subtitle">Review applications that have completed government verification and are awaiting final approval.</p>
          ...
          <div className="table-responsive">
            ...
            {!isLoading && !error && filteredApplications.length === 0 && (
              <div className="empty-state" style={{ 
                ...
                <p style={{ margin: '0 0 24px 0', color: 'var(--text-muted)', fontSize: '0.95rem' }}>There are no applications pending approval at this time.</p>
              </div>
            )}
          </div>
        </div>
