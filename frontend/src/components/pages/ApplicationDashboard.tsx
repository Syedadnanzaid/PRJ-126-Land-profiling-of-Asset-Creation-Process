...
const ApplicationDashboard = () => {
  ...
  const fetchApplications = async () => {
    ...
    try {
      ...
      setError("Unable to load land applications. Please try again.");
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <section className="dashboard-hero" style={{ minHeight: '140px', padding: '24px 40px' }}>
        <div className="hero-content">
          <p className="hero-date" style={{ color: 'var(--text-muted)' }}>
            Dashboard &gt; <span style={{ color: 'var(--accent-color)' }}>Applications</span>
          </p>
          <h1 className="hero-welcome">Land Applications</h1>
          <p className="hero-subtitle">Create, manage and track all land application workflows.</p>
        ...
        <div className="hero-actions">
          <button className="btn-primary" onClick={() => navigate('/applications/new')}>
            <span style={{ fontSize: '1.4rem', lineHeight: '1' }}>+</span> Create Application
          </button>
        </div>
      </section>
      ...
      <section className="dashboard-widget" style={{ padding: 0, backgroundColor: 'transparent', border: 'none', boxShadow: 'none' }}>
        ...
        <div className="assets-toolbar">
          ...
          <div className="toolbar-right">
            <span className="toolbar-count">Showing {applications.length > 0 ? 1 : 0}–{applications.length} of {applications.length} applications</span>
          ...
          <div className="table-responsive">
            ...
            {!isLoading && !error && applications.length === 0 && (
              <div className="empty-state" style={{ 
                ...
                <p style={{ margin: '0 0 12px 0', fontSize: '1.1rem', fontWeight: 600, color: 'var(--text-main)' }}>No land applications available yet.</p>
                <p style={{ margin: '0 0 24px 0', color: 'var(--text-muted)', fontSize: '0.95rem' }}>Get started by creating a new land application.</p>
                <button className="btn-primary" onClick={() => navigate('/applications/new')}>
                  <span style={{ fontSize: '1.4rem', lineHeight: '1' }}>+</span> Create Application
                </button>
              </div>
        ...
        <div className="pagination-footer">
          <div className="pagination-left">
            <span style={{ color: 'var(--text-muted)', fontSize: '0.9rem' }}>
              Showing {applications.length > 0 ? 1 : 0}–{applications.length} of {applications.length} applications
            </span>
